...
// Minimal mode constants
const MINIMAL_SCROLL_THRESHOLD = 0.92;
const MINIMAL_AUTO_HIDE = 3_000;

interface WhatsAppFABProps {
  mode?: "default" | "minimal";
}

const WhatsAppFAB = ({ mode = "default" }: WhatsAppFABProps) => {
  const [showTooltip, setShowTooltip] = useState(false);
  const [messageIndex, setMessageIndex] = useState(0);
  const [visible, setVisible] = useState(mode === "default");
  ...
  const minimalHideTimer = useRef<ReturnType<typeof setTimeout>>();
  ...
  }, [showNextTooltip, mode]);

  // === DEFAULT MODE: Idle trigger ===
  ...
  }, [resetIdle, isPermanentlyDismissed, mode]);

  // === MINIMAL MODE: Show only near footer, auto-hide after 3s ===
  useEffect(() => {
    if (mode !== "minimal") return;
    const onScroll = () => {
      const progress =
        window.scrollY / (document.body.scrollHeight - window.innerHeight);
      if (progress >= MINIMAL_SCROLL_THRESHOLD) {
        setVisible(true);
        clearTimeout(minimalHideTimer.current);
        minimalHideTimer.current = setTimeout(() => setVisible(false), MINIMAL_AUTO_HIDE);
      }
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => {
  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          {/* Tooltip — default mode only */}
          <AnimatePresence>
            {mode === "default" && showTooltip && (
              <motion.div
                role="complementary"
                aria-live="polite"
